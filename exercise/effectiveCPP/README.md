...
- 这个条款的目的就是就是为了减少默认构造函数的调用，直接拷贝构造肯定比默认构造+拷贝赋值快。
- 对于循环而言，就是要考虑构造成本和赋值成本的差异。

## 条款27 尽量少做转性动作

- 常用的转型操作：
    - const_cast：去const
    - dynamic_cast: 安全向下转型，决定某个对象是否归于继承体系中的某个类型
    - reinterpret_cast：实际效果取决于编译器，因此不可移植。可以将pointer to int 转型为int
    - static_cast：强迫隐式转换
- 旧式转型唯一的使用时机，调用一个explicit构造函数将一个对象传递给一个函数时。  
   ```c++
   dosomework(Widget(15));
   ```
- 单一对象可能有多个地址，如以base*指向一个目标，和以derived*指向同一个目标时的地址是不一样的。
- 在派生类中调用基类的函数时的做法：
    ```c++
    class specialWindow: public Window{
    public:
        virtual void onResize(){
            Window::onReize();//这里会作用在当前对象，注意不能使用转型去做。
        }
    }
    ```
